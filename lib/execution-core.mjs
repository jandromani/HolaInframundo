...
function latestMetricTimestamp(x){
  const values=[x?.asof,x?.intraday?.asof].map(v=>new Date(v||0).getTime()).filter(Number.isFinite);
  return values.length?Math.max(...values):0;
}
export function usRegularMarketSession(now=new Date()){
  const parts=Object.fromEntries(new Intl.DateTimeFormat('en-US',{
    timeZone:'America/New_York',weekday:'short',hour:'2-digit',minute:'2-digit',hourCycle:'h23'
  }).formatToParts(now).filter(x=>x.type!=='literal').map(x=>[x.type,x.value]));
  const weekday=parts.weekday||'',hour=Number(parts.hour),minute=Number(parts.minute),mins=hour*60+minute;
  const weekdayOpen=['Mon','Tue','Wed','Thu','Fri'].includes(weekday);
  const regularOpen=weekdayOpen&&mins>=570&&mins<960;
  return {
    regular_open:regularOpen,
    label:regularOpen?'OPEN':(!weekdayOpen?'CLOSED_WEEKEND':'CLOSED_OFF_HOURS'),
    weekday,hour,minute,timezone:'America/New_York'
  };
}
export function marketRegime(context={},policy={},now=new Date()){
  ...
  const maxAge=Number(p.max_snapshot_age_hours??2),minFresh=Number(p.min_fresh_benchmarks??3),nowMs=now.getTime();
  const names=['spx','nasdaq','qqq','iwm'],rows=[spx,nas,qqq,iwm];
  const ages={};let freshCount=0;
  rows.forEach((x,i)=>{
    const ts=latestMetricTimestamp(x);
    const age=ts?Math.max(0,(nowMs-ts)/3600000):Infinity;
    ages[names[i]]=Number.isFinite(age)?+age.toFixed(2):null;
    if(Number.isFinite(age)&&age<=maxAge)freshCount++;
  });
  const marketFresh=p.require_fresh_market===false?true:freshCount>=minFresh;
  const regimeAllowed=(p.entry_allowed||['RISK_ON','SELECTIVE']).includes(regime);
  const session=usRegularMarketSession(now);
  const sessionAllowed=p.require_regular_session===false?true:session.regular_open;
  return {
    regime,score,
    entry_allowed:Boolean(regimeAllowed&&marketFresh&&sessionAllowed),
    block_reason:!sessionAllowed?'MARKET_CLOSED':(!marketFresh?'STALE_MARKET_DATA':(!regimeAllowed?'MARKET_REGIME_BLOCK':null)),
    session,
    freshness:{required:p.require_fresh_market!==false,max_age_hours:maxAge,min_fresh_benchmarks:minFresh,fresh_count:freshCount,total:4,fresh:marketFresh,ages_hours:ages},
    signals:{spx,nasdaq:nas,qqq,iwm}
  };
}
...
export function candidateMomentumEligible(strategy,candidate,policy={}){
  const m=candidate?.market||{},cfg=policy?.selection?.candidate_momentum||{};
  const score=Number(m.score||0),rel5=Number(m.rel5||0),rel2h=Number(m.rel2h||0);
  if(strategy?.action==='DEPLOY_WINDOW'){
    if(score<Number(cfg.deploy_min_market_score??60)) return {ok:false,reason:'COMPANY_MARKET_SCORE_WEAK'};
    if(cfg.deploy_requires_above20!==false&&m.above20!==true) return {ok:false,reason:'COMPANY_BELOW_SMA20'};
    if(cfg.deploy_requires_positive_rel5!==false&&rel5<=Number(cfg.min_relative_5d??0)) return {ok:false,reason:'COMPANY_RELATIVE_5D_WEAK'};
    return {ok:true,reason:'DEPLOY_MOMENTUM_CONFIRMED'};
  }
  if(score<Number(cfg.scout_min_market_score??50)) return {ok:false,reason:'COMPANY_MARKET_SCORE_WEAK'};
  if(cfg.scout_requires_positive_rel2h_or_rel5!==false&&!(rel2h>0||rel5>Number(cfg.min_relative_5d??0))) return {ok:false,reason:'COMPANY_NO_EARLY_RELATIVE_STRENGTH'};
  return {ok:true,reason:'SCOUT_MOMENTUM_CONFIRMED'};
}
...
export function sizeOrder({strategy,candidate,positions=[],cashUsd,regime,policy}){
  ...
  if(!regime?.entry_allowed) return {usd:0,reason:regime?.block_reason||'MARKET_REGIME_BLOCK'};
  const momentum=candidateMomentumEligible(strategy,candidate,policy);
  if(!momentum.ok) return {usd:0,reason:momentum.reason};
  ...
  return {usd:+usd.toFixed(2),reason:'OK',group,reference_price:Number(candidate.market.price),chase_limit_pct:Number(exec.price_chase_limit_pct??2.5),momentum_reason:momentum.reason};
}
...
export function candidateForStrategy(strategy,positions=[],policy={}){
  const pool=(strategy?.ranked_candidates?.length?strategy.ranked_candidates:strategy?.top5)||[];
  for(const c of pool){
    ...
    if(!candidateMomentumEligible(strategy,c,policy).ok) continue;
    return c;
  }
  return null;
}
