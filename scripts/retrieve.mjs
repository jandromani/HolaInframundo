...
async function gdelt(query,windowHours){
  const end=new Date(); const start=new Date(end.getTime()-windowHours*3600e3);
  ...
  const r=await safeFetch(url); const j=await r.json();
  return (j.articles||[]).slice(0,MAX).map(a=>({
    engine:'gdelt',title:a.title||'',url:a.url||'',domain:a.domain||domainOf(a.url),source_name:a.domain||domainOf(a.url),published_at:a.seendate||'',snippet:'',language:a.language||'',source_country:a.sourcecountry||''
  }));
}
...
async function googleNews(query,windowHours){
  ...
  const r=await safeFetch(url); const xml=await r.text();
  const items=[...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].slice(0,MAX).map(x=>x[1]);
  return items.map(block=>{const src=sourceTag(block);const link=xmlTag(block,'link');return {
    engine:'google_news',title:xmlTag(block,'title'),url:link,domain:domainOf(src.url)||domainOf(link),source_name:src.name||domainOf(link),published_at:xmlTag(block,'pubDate'),snippet:xmlTag(block,'description'),language:'',source_country:''
  }});
}
...
const output={version:'2.0.0',run_id:runId,generated_at:now.toISOString(),window_hours:queryCfg.defaults?.window_hours||30,mechanisms:{},errors:[],request_count:0};
for(const m of selected){
  const qset=queryCfg.mechanisms[m.id];
  if(!qset){output.errors.push({mechanism_id:m.id,error:'Missing V2 query config'});continue;}
  const bucket={id:m.id,label:m.label,tier:m.tier,queries:[],candidates:[]};
  ...
  for(const phase of phasesFor(m,qset)){
    for(const query of qset[phase]){
      ...
      for(const engine of queryCfg.defaults?.retrievers||['gdelt','google_news']){
        try{
          const rows=engine==='gdelt'?await gdelt(query.q,output.window_hours):await googleNews(query.q,output.window_hours);
          output.request_count++;
          trace.engines[engine]={ok:true,count:rows.length};
          for(const row of rows){
            const key=(row.url||row.title).toLowerCase().replace(/[?#].*$/,'');
            if(!key)continue;
            ...
            if(!existing)dedupe.set(key,item);
            else existing.matched_queries=[...new Set([...(existing.matched_queries||[existing.query_id]),queryId])];
          }
        }catch(e){
          output.request_count++;
          trace.engines[engine]={ok:false,error:String(e.message||e)};
          output.errors.push({mechanism_id:m.id,query_id:queryId,engine,error:String(e.message||e)});
        }
        await sleep(220);
      }
  ...
  bucket.candidates=[...dedupe.values()].sort((a,b)=>String(b.published_at).localeCompare(String(a.published_at))).slice(0,36);
  output.mechanisms[m.id]=bucket;
}

await fs.mkdir('data/retrieval',{recursive:true});
await fs.writeFile('data/retrieval/latest.json',JSON.stringify(output,null,2)+'\n');
