...
const scoreConvexity=mc=>{if(!Number.isFinite(mc)||mc<=0)return null;if(mc<5e8)return 94;if(mc<1e9)return 90;if(mc<3e9)return 82;if(mc<1e10)return 72;if(mc<3e10)return 62;if(mc<1e11)return 52;if(mc<3e11)return 44;return 38};
...
export function deriveFinancialScores(fundamental={},price=null,mode='CONTRACT_BACKLOG'){
  ...
  const revenue=Number(x.revenue),prevRevenue=Number(x.revenue_prev),op=Number(x.operating_income),prevOp=Number(x.operating_income_prev),cfo=Number(x.cfo),capex=Math.abs(Number(x.capex)),assets=Number(x.assets),liabilities=Number(x.liabilities),cash=Number(x.cash),debt=Math.abs(Number(x.debt)),explicitFcf=Number(x.free_cash_flow);

  let operatingLeverage=null;
  if(Number.isFinite(revenue)&&revenue!==0&&Number.isFinite(op)){
    const margin=op/revenue,prevMargin=Number.isFinite(prevRevenue)&&prevRevenue!==0&&Number.isFinite(prevOp)?prevOp/prevRevenue:null,revGrowth=Number.isFinite(prevRevenue)&&prevRevenue!==0?revenue/prevRevenue-1:null,opGrowth=Number.isFinite(prevOp)&&Math.abs(prevOp)>1?op/prevOp-1:null,sensitivity=revGrowth!=null&&opGrowth!=null?Math.min(5,Math.abs(opGrowth)/Math.max(.05,Math.abs(revGrowth))):null;
  ...
  const fcf=Number.isFinite(explicitFcf)?explicitFcf:Number.isFinite(cfo)&&Number.isFinite(capex)?cfo-capex:null,fcfMargin=fcf!=null&&Number.isFinite(revenue)&&revenue!==0?fcf/revenue*100:null,cashConversion=scoreFcfMargin(fcfMargin);
  const liabRatio=ratio(liabilities,assets),cashDebt=Number.isFinite(cash)&&Number.isFinite(debt)&&debt>0?cash/debt:null,balance=avg([scoreLiabilities(liabRatio),scoreCashDebt(cashDebt)].filter(Number.isFinite));
  const capexCoverage=Number.isFinite(cfo)&&Number.isFinite(capex)&&capex>0?cfo/capex:null,capexReadiness=scoreCapexCoverage(capexCoverage),shares=Number(x.shares),px=Number(price??fundamental.price),marketCap=Number.isFinite(shares)&&shares>0&&Number.isFinite(px)&&px>0?shares*px:Number(fundamental.market_cap),convexity=scoreConvexity(marketCap);
  ...
  return {
    ...
    metrics:{operating_margin:Number.isFinite(revenue)&&revenue!==0&&Number.isFinite(op)?+(op/revenue*100).toFixed(2):null,fcf_margin:fcfMargin==null?null:+fcfMargin.toFixed(2),liabilities_to_assets:liabRatio==null?null:+liabRatio.toFixed(3),cash_to_debt:cashDebt==null?null:+cashDebt.toFixed(2),cfo_to_capex:capexCoverage==null?null:+capexCoverage.toFixed(2),market_cap:Number.isFinite(marketCap)?Math.round(marketCap):null}
  };
}
