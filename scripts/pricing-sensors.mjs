...
import {collectAisHormuz,collectEia,collectMofJgb,collectMofFlows,collectCftc,collectUsaspending,collectSam,collectUkmto,collectEntsoe,collectComtrade,clamp} from './pricing-adapters.mjs';
...
function familyFallback(s){if(['AISSTREAM','EIA','UKMTO','ENTSOE','UN_COMTRADE'].includes(s))return'PHYSICAL';if(['USA_SPENDING','SAM_GOV'].includes(s))return'PROCUREMENT';if(['MOF_JGB','MOF_FLOWS','CFTC_COT'].includes(s))return'MARKET';return'UNKNOWN'}
...
const jobs={
 AISSTREAM:collectAisHormuz(cfg,memory),
 EIA:collectEia(cfg),
 MOF_JGB:collectMofJgb(cfg),
 MOF_FLOWS:collectMofFlows(cfg),
 CFTC_COT:collectCftc(cfg),
 USA_SPENDING:collectUsaspending(cfg),
 SAM_GOV:collectSam(cfg),
 UKMTO:collectUkmto(cfg),
 ENTSOE:collectEntsoe(cfg),
 UN_COMTRADE:collectComtrade(cfg)
};
...
const mechanismSources={
 DIESEL_REFINING_CRISIS:['EIA','CFTC_COT'],PRODUCT_TANKER_TONMILES:['AISSTREAM','CFTC_COT'],CRUDE_TANKER_DISLOCATION:['AISSTREAM','CFTC_COT'],ATLANTIC_BARREL_PREMIUM:['AISSTREAM','EIA'],CEYHAN_BYPASS:['AISSTREAM','UKMTO'],LNG_REROUTING:['AISSTREAM','EIA'],
 ARSENAL_DEPLETION:['USA_SPENDING','SAM_GOV'],ROCKET_MOTOR_SHORTAGE:['USA_SPENDING','SAM_GOV'],ENERGETICS_SHORTAGE:['USA_SPENDING','SAM_GOV','UN_COMTRADE'],CHEAP_DRONE_SCALING:['USA_SPENDING','SAM_GOV'],COUNTER_UAS_COST_CURVE:['USA_SPENDING','SAM_GOV'],UNDERWATER_SECURITY:['UKMTO','AISSTREAM','USA_SPENDING'],MILITARY_ENERGY_AUTONOMY:['USA_SPENDING','SAM_GOV'],EUROPE_STRATEGIC_AUTONOMY:['USA_SPENDING','UN_COMTRADE'],
 AI_TIME_TO_POWER:['EIA','ENTSOE'],AI_GHOST_DEMAND:['ENTSOE'],BEHIND_METER_AI_POWER:['EIA','ENTSOE'],ROBOTICS_ACTUATOR_SCALE:['UN_COMTRADE'],ROBOTICS_DECOUPLING:['UN_COMTRADE'],STRATEGIC_ALUMINIUM:['UN_COMTRADE','CFTC_COT'],RARE_EARTH_MAGNET_SHORTAGE:['UN_COMTRADE'],NUCLEAR_FUEL_SOVEREIGNTY:['USA_SPENDING','EIA'],JAPAN_CARRY_UNWIND:['MOF_JGB','MOF_FLOWS','CFTC_COT'],GLOBAL_REFINANCING_STRESS:['CFTC_COT'],OT_CYBER_SHOCK:['USA_SPENDING','SAM_GOV'],OIL_SERVICE_CAPEX_SECOND_WAVE:['EIA','CFTC_COT']
};
function rowFor(source){const o=observations[source];return {source,family:o?.family||cfg.sources[source]?.family||familyFallback(source),access:cfg.sources[source]?.access||null,status:o?.status||'UNOBSERVED',score:Number.isFinite(o?.pricing_score)?o.pricing_score:null,metric:o?.metric||null,quality:o?.quality||null,weight:sourceWeights[source]||1}}
...
const mechanisms={};
for(const id of Object.keys(cfg.mechanisms||{})){const requested=mechanismSources[id]||cfg.mechanisms[id]||[],rows=requested.map(rowFor),agg=aggregate(rows);mechanisms[id]={...agg,sources:rows,total_sources:rows.length,pricing_method:agg.observed_sources?'PRIMARY_MULTI_SENSOR':'NO_PRIMARY_OBSERVATION',blind_reasons:rows.filter(x=>!Number.isFinite(x.score)).map(x=>`${x.source}:${x.status}`)}}

const out={version:'4.3.0',generated_at:nowIso,observations,mechanisms,coverage:{mechanisms:Object.keys(mechanisms).length,live_sources:Object.values(observations).filter(x=>x.status==='OK').length,scored_sources:Object.values(observations).filter(x=>Number.isFinite(x.pricing_score)).length,configured_sources:Object.keys(cfg.sources).length,blocks:{ais_hormuz:observations.AISSTREAM?.status||'UNKNOWN',eia:observations.EIA?.status||'UNKNOWN',mof_japan:[observations.MOF_JGB?.status,observations.MOF_FLOWS?.status],us_procurement:[observations.USA_SPENDING?.status,observations.SAM_GOV?.status],cftc:observations.CFTC_COT?.status||'UNKNOWN',ukmto:observations.UKMTO?.status||'UNKNOWN',entsoe:observations.ENTSOE?.status||'UNKNOWN',comtrade:observations.UN_COMTRADE?.status||'UNKNOWN'}}};
