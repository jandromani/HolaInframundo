...
import {improveObservations} from './pricing-quality.mjs';
...
const memory=JSON.parse(await fs.readFile(memoryPath,'utf8').catch(()=>'{"version":"4.4.0","hormuz_density":[],"source_series":{}}'));
const observations={};
...
await improveObservations(observations);
for(const [source,o] of Object.entries(observations)){o.source=source;o.family=o.family||cfg.sources[source]?.family||familyFallback(source);o.observed_at=o.observed_at||nowIso}
...
memory.updated_at=nowIso;memory.version='4.4.0';

const sourceWeights={AISSTREAM:1.35,EIA:1.25,MOF_JGB:1.35,MOF_FLOWS:1.25,CFTC_COT:.85,USA_SPENDING:1.2,SAM_GOV:1.05,UKMTO:1.2,ENTSOE:1.15,UN_COMTRADE:.9,ACER_LNG:1.25,KALSHI:.65};
...
const out={version:'4.4.0',generated_at:nowIso,observations,mechanisms,coverage:{mechanisms:Object.keys(mechanisms).length,live_sources:Object.values(observations).filter(x=>x.status==='OK').length,scored_sources:Object.keys(observations).filter(source=>source==='KALSHI'?Object.values(observations.KALSHI?.mechanisms||{}).some(x=>Number.isFinite(x.pricing_score)):Number.isFinite(observations[source]?.pricing_score)).length,configured_sources:Object.keys(cfg.sources).length,blocks:{ais_hormuz:observations.AISSTREAM?.status||'UNKNOWN',eia:observations.EIA?.status||'UNKNOWN',mof_japan:[observations.MOF_JGB?.status,observations.MOF_FLOWS?.status],us_procurement:[observations.USA_SPENDING?.status,observations.SAM_GOV?.status],cftc:observations.CFTC_COT?.status||'UNKNOWN',ukmto:observations.UKMTO?.status||'UNKNOWN',entsoe:observations.ENTSOE?.status||'UNKNOWN',comtrade:observations.UN_COMTRADE?.status||'UNKNOWN'},retained:{acer_lng:observations.ACER_LNG?.status||'UNKNOWN',kalshi:observations.KALSHI?.status||'UNKNOWN'},quality:{kalshi_rejected_semantic_hits:observations.KALSHI?.rejected_semantic_hits||0,eia_public_fallback:Boolean(observations.EIA?.fallback),ukmto_public_fallback:Boolean(observations.UKMTO?.fallback),mof_public_fallback:Boolean(observations.MOF_JGB?.fallback)}}};
await fs.writeFile('data/pricing-sensors.json',JSON.stringify(out,null,2)+'\n');await fs.writeFile(memoryPath,JSON.stringify(memory,null,2)+'\n');
console.log(`pricing sensors v4.4: live=${out.coverage.live_sources} scored=${out.coverage.scored_sources} rejectedKalshi=${out.coverage.quality.kalshi_rejected_semantic_hits} blocks=8 mechanisms=${out.coverage.mechanisms}`);
