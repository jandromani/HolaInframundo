...
import {calendarReturn,rangeStats} from './investment-analytics-core.mjs';
...
function dailyMetric(x,ticker){
  const q=x.indicators?.quote?.[0]||{},pairs=(q.close||[]).map((c,i)=>({c,v:q.volume?.[i],t:x.timestamp?.[i]})).filter(z=>Number.isFinite(z.c)&&Number.isFinite(z.t)),closes=pairs.map(z=>z.c),vols=pairs.map(z=>Number.isFinite(z.v)?z.v:0);if(closes.length<21)throw new Error('Insufficient daily history');
  const recentVol=vols.slice(-60),volStd=sd(recentVol),volumeZ=volStd&&vols.length?((vols.at(-1)-mean(recentVol))/volStd):0,price=closes.at(-1),s20=sma(closes,20),s50=sma(closes,50),s200=sma(closes,200),range=rangeStats(pairs);
  return {ticker,asof:new Date((pairs.at(-1)?.t||Date.now()/1000)*1000).toISOString(),price:+price.toFixed(4),ret1:ret(closes,1)==null?null:+ret(closes,1).toFixed(2),ret5:ret(closes,5)==null?null:+ret(closes,5).toFixed(2),ret20:ret(closes,20)==null?null:+ret(closes,20).toFixed(2),ret7:calendarReturn(pairs,7),ret30:calendarReturn(pairs,30),ret90:calendarReturn(pairs,90),ret180:calendarReturn(pairs,180),high52:range.high52,low52:range.low52,drawdown52:range.drawdown52,volumeZ:+volumeZ.toFixed(2),sma20:s20==null?null:+s20.toFixed(4),sma50:s50==null?null:+s50.toFixed(4),sma200:s200==null?null:+s200.toFixed(4),above20:s20!=null?price>s20:null,above50:s50!=null?price>s50:null,above200:s200!=null?price>s200:null};
}
...
console.log(`market metrics v2.7: ${Object.keys(out.metrics).length}/${tickers.length}, context ${Object.keys(out.context).length}/${contextTickers.length}, intraday=${intradayOk}`);
