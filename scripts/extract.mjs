...
const modelHealth=JSON.parse(await fs.readFile(healthPath,'utf8').catch(()=>'{"version":"2.3.0","models":{}}'));modelHealth.version='2.3.0';
const now=new Date(),day=now.toISOString().slice(0,10);
budget.days[day]??={llm:0,web:0,extraction:0,verifier:0,extractor_paid_usd:0};budget.days[day].extraction??=Math.max(0,(budget.days[day].llm||0)-(budget.days[day].verifier||0));budget.days[day].verifier??=budget.days[day].web||0;budget.days[day].extractor_paid_usd??=0;
const envModel=process.env.OPENROUTER_MODEL,PRIMARY=envModel||policy.model||'openai/gpt-oss-20b',MODEL_POOL=[...new Set([PRIMARY,...(policy.model_pool||[]),'openrouter/free'].filter(Boolean))],PAID_CAP=Number(policy.budgets?.max_paid_extractor_usd_per_day||.05);
...
function cleanJson(s){let x=String(s||'').trim().replace(/^```(?:json)?\s*/i,'').replace(/\s*```$/,'');const a=x.indexOf('{'),b=x.lastIndexOf('}');if(a>=0&&b>a)x=x.slice(a,b+1);return JSON.parse(x)}
function validParsed(x){return x&&typeof x.summary==='string'&&Array.isArray(x.items)&&Array.isArray(x.expected_next)&&Array.isArray(x.invalidation)}
function health(model){return modelHealth.models[model]??={success:0,fail:0,http404:0,http429:0,schema_fail:0,total_ms:0,last_error:null,last_success:null,last_used:null}}
function isFree(model){return model==='openrouter/free'||model.endsWith(':free')}
...
async function rawCall(model,body){
  ...
  while(true){
    let r,text;
    try{r=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${API_KEY}`,'Content-Type':'application/json','HTTP-Referer':'https://gearwatch-market-causal-memory-jandromanis-projects.vercel.app','X-Title':'GearWatch Extractor'},body:JSON.stringify({...body,model}),signal:AbortSignal.timeout(isFree(model)?35000:45000)});text=await r.text()}catch(e){h.fail++;h.last_error=String(e.message||e);throw e}
    if(r.ok){h.success++;h.total_ms+=Date.now()-t;h.last_success=new Date().toISOString();return JSON.parse(text)}
    if(r.status===404)h.http404++;if(r.status===429)h.http429++;h.last_error=`${r.status}:${text.slice(0,180)}`;
    if(r.status>=500&&retryOther<(policy.scheduler?.max_retry_other??1)&&!isFree(model)){retryOther++;await sleep(900);continue}
    h.fail++;throw Object.assign(new Error(`OpenRouter ${r.status}: ${text.slice(0,300)}`),{status:r.status})
  }
}
async function callWithPool(messages){
  const failures=[];
  for(const model of MODEL_POOL){
    if(!isFree(model)&&budget.days[day].extractor_paid_usd>=PAID_CAP){failures.push(`${model}: PAID_CAP_REACHED`);continue}
    const h=health(model);if(h.http404>0&&!h.last_success&&model!==PRIMARY){failures.push(`${model}: HEALTH_SKIP_404`);continue}
    try{
      const response=await rawCall(model,{temperature:0,max_tokens:1200,messages,provider:{require_parameters:true},response_format:{type:'json_schema',json_schema:{name:'gearwatch_extraction',strict:true,schema}}});
      const parsed=cleanJson(response.choices?.[0]?.message?.content||'{}');if(!validParsed(parsed)){h.schema_fail++;throw new Error('JSON_SCHEMA_CONTRACT_INVALID')}
      const cost=costOf(response);if(!isFree(model)&&cost){budget.days[day].extractor_paid_usd=+(budget.days[day].extractor_paid_usd+cost).toFixed(6)}
      return {response,parsed,requested_model:model,format_mode:'json_schema',cost}
    }catch(e){failures.push(`${model}: ${String(e.message||e).slice(0,240)}`)}
  }
  throw new Error(`MODEL_POOL_EXHAUSTED | ${failures.join(' | ').slice(0,1200)}`)
}
...
const candidates=mechCfg.mechanisms.filter(m=>retrieval.mechanisms?.[m.id]).sort((a,b)=>priority(b)-priority(a)||a.id.localeCompare(b.id)),chosen=new Set(candidates.slice(0,runCap).map(m=>m.id));
const results={version:'2.3.0',run_id:retrieval.run_id,generated_at:now.toISOString(),model_pool:MODEL_POOL,quota:{extraction_daily_cap:extractionDailyCap,remaining_before:remainingDaily,run_cap:runCap,selected:candidates.length,paid_cap_usd:PAID_CAP,paid_before_usd:budget.days[day].extractor_paid_usd},mechanisms:{},errors:[],deferred:[]};let runRequests=0;
for(const m of candidates){
  const bucket=retrieval.mechanisms?.[m.id];if(!chosen.has(m.id)){results.deferred.push({mechanism_id:m.id,reason:'QUOTA_ROTATION',priority:+priority(m).toFixed(1)});continue}
  if(!(bucket.candidates||[]).length){results.mechanisms[m.id]={id:m.id,label:m.label,tier:m.tier,summary:'No fresh deterministic candidates in this cycle.',evidence:[],expected_next:[],invalidation:[],model:null,usage:null,candidate_count:0,query_trace:bucket.queries||[]};continue}
