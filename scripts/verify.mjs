...
const quota=JSON.parse(await fs.readFile('data/openrouter-quota.json','utf8').catch(()=>'{"safe_daily_requests":50,"verifier_daily_reserve":4}'));
...
const now=new Date(),day=now.toISOString().slice(0,10);budget.days[day]??={llm:0,web:0,extraction:0,verifier:0};budget.days[day].verifier??=budget.days[day].web||0;budget.days[day].extraction??=Math.max(0,(budget.days[day].llm||0)-(budget.days[day].verifier||0));
...
const dailyReserve=Math.min(policy.budgets?.max_web_verifiers_per_day||12,Number(quota.verifier_daily_reserve||4));
const remaining=Math.max(0,dailyReserve-(budget.days[day].verifier||0));
const maxRun=Math.min(policy.budgets?.max_web_verifiers_per_run||4,Math.ceil(remaining/slotsLeft()));
const chosen=candidates.slice(0,maxRun);
...
const out={version:'2.0.0',run_id:extraction.run_id,generated_at:now.toISOString(),quota:{daily_reserve:dailyReserve,remaining_before:remaining,run_cap:maxRun,candidates:candidates.length},mechanisms:{},errors:[]};let used=0;
for(const o of chosen){
  if((budget.days[day].verifier||0)>=dailyReserve||(budget.days[day].llm||0)>=Number(quota.safe_daily_requests||50))break;
  try{
    const {response,parsed,grounded}=await call(o);used++;budget.days[day].web++;budget.days[day].llm++;budget.days[day].verifier++;
...
console.log(`verification ${extraction.run_id}: ${used}/${maxRun} web verifiers, daily ${budget.days[day].verifier}/${dailyReserve}, errors=${out.errors.length}`);
