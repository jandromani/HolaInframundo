...
const previousDiscovery=JSON.parse(await fs.readFile('data/discovery/latest.json','utf8').catch(()=>'{"selected":[]}'));
const previouslySelected=new Set(previousDiscovery.selected||[]);
...
function priority(m,b){const p=previous.mechanisms?.[m.id],count=(b?.candidates||[]).length;return ({A:100,B:55,C:20}[m.tier]||0)+({ACTIVE:90,ARMING:75,WATCH:50,STALE:65,UNKNOWN:35,DORMANT:15}[p?.state]||30)+(p?.alpha?.eligible?50:0)+(count===0?90:count===1?40:0)-(previouslySelected.has(m.id)?70:0)}
const threshold=policy.retrieval?.fallback_when_candidates_below||2;
const need=mechCfg.mechanisms.map(m=>({m,b:retrieval.mechanisms?.[m.id]})).filter(x=>x.b&&(x.b.candidates||[]).length<threshold).sort((a,b)=>priority(b.m,b.b)-priority(a.m,a.b));
const chosen=need.slice(0,runCap);const trace={version:'3.1.0',run_id:retrieval.run_id,generated_at:now.toISOString(),model:MODEL,candidates:need.length,selected:chosen.map(x=>x.m.id),previously_selected:[...previouslySelected],searches:[],errors:[]};
for(const {m,b} of chosen){
  const q=(b.queries||[]).find(x=>x.backstop)||(b.queries||[]).find(x=>x.phase==='LEAD')||(b.queries||[])[0];if(!q)continue;
  ...
  try{
    ...
    let added=0;for(const a of anns){if(!a.url)continue;const key=a.url.toLowerCase().replace(/[?#].*$/,'');if((b.candidates||[]).some(x=>(x.url||'').toLowerCase().replace(/[?#].*$/,'')===key))continue;b.candidates.push({id:hash(`${q.query_id}|${a.url}|${a.title}`),mechanism_id:m.id,query_id:q.query_id,phase:q.phase||'LEAD',signal:q.signal||'WEB_RESCUE',query:q.q,engine:'openrouter_web_fallback',title:a.title||a.content?.slice(0,140)||'Web discovery',url:a.url,domain:(()=>{try{return new URL(a.url).hostname.replace(/^www\./,'')}catch{return ''}})(),source_name:a.title||'web',published_at:null,published_at_known:false,snippet:a.content||'',retrieved_at:now.toISOString(),fallback:true});added++}
    trace.searches.push({mechanism_id:m.id,query_id:q.query_id,q:q.q,annotations:anns.length,added,actual_model:response.model,usage:response.usage||null,priority:priority(m,b)});
...
retrieval.discovery_fallback={run_cap:runCap,needed:need.length,used:trace.searches.length,errors:trace.errors.length,rotation:true,generated_at:now.toISOString()};
await fs.writeFile(retrievalPath,JSON.stringify(retrieval,null,2)+'\n');await fs.mkdir('data/discovery',{recursive:true});await fs.writeFile('data/discovery/latest.json',JSON.stringify(trace,null,2)+'\n');await fs.writeFile(`data/discovery/${retrieval.run_id}.json`,JSON.stringify(trace,null,2)+'\n');await fs.writeFile(budgetPath,JSON.stringify(budget,null,2)+'\n');console.log(`discovery fallback: ${trace.searches.length}/${runCap} searches, ${trace.searches.reduce((a,x)=>a+x.added,0)} candidates added, blind=${need.length}, rotated=true, errors=${trace.errors.length}`);
