...
assert.equal(cfg.version,'4.4.0');for(const m of mechs){assert.ok(Array.isArray(cfg.mechanisms[m.id])&&cfg.mechanisms[m.id].length>0,`missing pricing sensors for ${m.id}`);for(const s of cfg.mechanisms[m.id])assert.ok(cfg.sources[s],`unknown source ${s} for ${m.id}`)}
...
console.log(`pricing sensor config v4.4 OK: ${mechs.length} mechanisms / ${Object.keys(cfg.sources).length} sources / 8 primary blocks`);
