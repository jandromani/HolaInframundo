...
const now=new Date(),nowIso=now.toISOString(),TTL=3*24*3600*1000,COLLECTOR_VERSION='2.8.1';
...
const out={version:'2.8.1',collector_version:COLLECTOR_VERSION,generated_at:nowIso,provider:'Yahoo fundamentals-timeseries (cached, unauthenticated) with deterministic fallback',companies:{...prev.companies},coverage:{seed_symbols:symbols.length,live:0,partial:0,structural_only:0,cache_hits:0,network_fetches:0,network_errors:0,avg_field_coverage:0},errors:[]};
...
function yahooUrl(symbol){
  // Yahoo expects one comma-separated `type=` value. This matches current yfinance.
...
async function fetchYahoo(symbol){
  const r=await fetch(yahooUrl(symbol),{headers:{'user-agent':'Mozilla/5.0 GearWatch/2.8.1','accept':'application/json','referer':`https://finance.yahoo.com/quote/${encodeURIComponent(symbol)}/financials`},signal:AbortSignal.timeout(22000)});
...
for(const symbol of symbols){
  ...
  try{const result=await fetchYahoo(symbol);out.companies[symbol]=parseYahoo(result,symbol);out.coverage.network_fetches++}
    ...
    if(fresh(cached))out.companies[symbol]={...cached,stale:true,last_error:msg};
...
const vals=Object.values(out.companies);out.coverage.live=vals.filter(x=>x.quality==='LIVE_YAHOO').length;out.coverage.partial=vals.filter(x=>x.quality==='PARTIAL_YAHOO').length;out.coverage.structural_only=vals.filter(x=>x.quality==='STRUCTURAL_ONLY').length;out.coverage.avg_field_coverage=vals.length?+(vals.reduce((a,x)=>a+Number(x.coverage||0),0)/vals.length).toFixed(3):0;
await fs.writeFile(path,JSON.stringify(out,null,2)+'\n');
console.log(`fundamentals: live=${out.coverage.live} partial=${out.coverage.partial} structural=${out.coverage.structural_only} fieldcov=${Math.round(out.coverage.avg_field_coverage*100)}% cache=${out.coverage.cache_hits} fetched=${out.coverage.network_fetches} errors=${out.coverage.network_errors}`);
