...
const now=new Date(),nowIso=now.toISOString(),TTL=3*24*3600*1000,COLLECTOR_VERSION='3.0.0';
...
const out={version:'3.0.0',collector_version:COLLECTOR_VERSION,generated_at:nowIso,provider:'Yahoo fundamentals-timeseries (cached, unauthenticated) with deterministic fallback',companies:{},coverage:{seed_symbols:symbols.length,live:0,partial:0,structural_only:0,cache_hits:0,network_fetches:0,network_errors:0,avg_field_coverage:0},errors:[]};
...
async function fetchYahoo(symbol){
  const r=await fetch(yahooUrl(symbol),{headers:{'user-agent':'Mozilla/5.0 GearWatch/3.0','accept':'application/json','referer':`https://finance.yahoo.com/quote/${encodeURIComponent(symbol)}/financials`},signal:AbortSignal.timeout(22000)});
...
function parseYahoo(result,symbol){
  ...
  const fin={revenue:revenue.value,revenue_prev:revenue.prev,operating_income:op.value,operating_income_prev:op.prev,cfo:cfo.value,capex:capex.value,assets:assets.value,liabilities:liabilities.value,cash:cash.value,debt:debt.value,shares:shares.value,shares_prev:shares.prev,free_cash_flow:fcf.value,ebitda:ebitda.value};
...
console.log(`fundamentals v3.0: universe=${vals.length}/${symbols.length} live=${out.coverage.live} partial=${out.coverage.partial} structural=${out.coverage.structural_only} fieldcov=${Math.round(out.coverage.avg_field_coverage*100)}% cache=${out.coverage.cache_hits} fetched=${out.coverage.network_fetches} errors=${out.coverage.network_errors}`);
